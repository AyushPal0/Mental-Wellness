...
const OCD_SEVERITY_LEVELS = [
  { score: 0, label: "None", description: "No significant OCD traits detected", color: "text-green-300" },
  { score: 20, label: "Mild", description: "Minor organizational preferences", color: "text-green-400" },
...
export default function GamePlayPage() {
  ...
  if (isCompleted) {
    return (
      <div className="min-h-screen text-white p-4 flex items-center justify-center relative overflow-hidden">
        {/* Video Background */}
        ...
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 p-6 rounded-2xl h-full">
          <div className="text-center mb-8">
            <div className="w-24 h-24 bg-gradient-to-r from-black to-gray-900 rounded-full flex items-center justify-center mx-auto mb-6">
              <svg className="w-12 h-12 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path>
              </svg>
            </div>
            
            <h2 className="text-3xl font-bold bg-gradient-to-r from-slate-300 to-gray-300 bg-clip-text text-transparent mb-2">
              OCD Assessment Complete
            </h2>
            <p className="text-slate-300">Based on your responses to {GAME_LEVELS.length} levels of disorder</p>
          </div>
          
          <div className="bg-black rounded-2xl p-6 mb-8">
            <div className="text-center mb-6">
              <div className="text-5xl font-bold mb-2 text-white">{ocdScore}/100</div>
              <div className={`text-2xl font-bold ${severityLevel.color}`}>{severityLevel.label} Severity</div>
              <p className="text-slate-300 mt-2">{severityLevel.description}</p>
            </div>
          ...
          <div className="bg-black rounded-2xl p-6 mb-8">
            <h3 className="text-lg font-bold mb-4 text-center text-white">Your Responses to Disorder</h3>
            <div className="space-y-3">
              {userResponses.map((response, index) => (
                <div key={index} className="flex items-center justify-between">
                  <span className="text-slate-300">Level {response.level} (Intensity: {response.intensity}/5):</span>
          ...
          <div className="bg-black bg-opacity-50 rounded-2xl p-4 mb-8">
          ...
          <div className="flex space-x-4">
            {isOnboarding ? (
                <button 
                  ...
                  className="flex-1 bg-gradient-to-r from-slate-600 to-gray-600 hover:from-slate-700 hover:to-gray-700 text-white font-bold py-4 rounded-xl transition-all duration-300"
                >
                ...
                <>
                    ...
                    <button 
                      ...
                      className="flex-1 bg-slate-700 hover:bg-slate-600 text-white font-bold py-4 rounded-xl transition-all duration-300"
                    >
                      Main Menu
                    </button>
                </>
    ...
    <div className="min-h-screen text-white p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto mb-8">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold bg-gradient-to-r from-slate-300 to-gray-300 bg-clip-text text-transparent">
              OCD Trigger Assessment
            </h1>
            <p className="text-slate-300 text-sm">Level {currentLevel + 1} of {GAME_LEVELS.length}: {currentLevelData.name}</p>
          </div>
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 p-6 rounded-2xl h-full">
            <span className="text-sm text-slate-300 mr-2">VIEWING TIME</span>
            <span className="font-bold text-xl text-white">{viewTime}s</span>
          </div>
        </div>
      </div>

      <div className="relative z-10 max-w-6xl mx-auto">
        <div className="bg-white/10 backdrop-blur-sm border border-white/30 p-6 rounded-2xl h-full">
          ...
          <div className="mt-4">
            <span className="text-sm text-slate-300">Disorder Intensity: </span>
        ...
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 p-6 rounded-2xl h-full">
          <div 
            className="game-table mx-auto relative bg-black border-2 border-gray-600 rounded-xl overflow-hidden shadow-2xl"
        ...
        {!hasSeenTable && (
          <div className="text-center">
            <button 
              ...
              className="bg-gradient-to-r from-slate-600 to-gray-600 hover:from-slate-700 hover:to-gray-700 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 text-lg"
            >
              I'm Ready to Answer
            </button>
          </div>
      ...
      {showTriggerQuestion && (
        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4">
          <div className="bg-gradient-to-br from-slate-800 to-gray-900 border border-slate-700 rounded-3xl p-8 max-w-2xl w-full shadow-2xl">
            <div className="text-center mb-8">
              ...
              <p className="text-slate-300 text-lg mb-2">{currentLevelData.description}</p>
              <p className="text-slate-400 text-sm">(Disorder level: {currentLevelData.disorderIntensity}/5)</p>
            </div>
            
            <div className="space-y-4">
              <button onClick={() => handleUserResponse(1)} className="w-full bg-slate-700 hover:bg-slate-600 text-white font-bold py-4 rounded-xl transition-all duration-300 text-lg">
                😌 Completely Comfortable
              </button>
              <button onClick={() => handleUserResponse(2)} className="w-full bg-gray-700 hover:bg-gray-600 text-white font-bold py-4 rounded-xl transition-all duration-300 text-lg">
      ...
      <style jsx>{`
        .bg-wood-pattern {
          background-image: url("data:image/svg+xml,%3Csvg width='100' height='100' viewBox='0 0 100 100' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M0 0h100v100H0z' fill='%23000000'/%3E%3Cpath d='M0 0l100 100M100 0L0 100' stroke='%23333333' stroke-width='1' opacity='0.2'/%3E%3C/svg%3E");
        }
      `}</style>
    </div>
  );
