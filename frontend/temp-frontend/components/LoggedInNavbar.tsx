...
export function LoggedInNavbar({ onProfileClick }: { onProfileClick?: () => void; }) {
  ...
  // Updated navLinks to use 'path' for comparison
  const navLinks = [
    { name: "Home", path: "/home", icon: <Home size={20} /> },
    { name: "Chat", path: "/chatbot", icon: <MessageSquare size={20} /> },
    { name: "Tasks", path: "/tasks", icon: <ClipboardCheck size={20} /> },
    { name: "Community", path: "/community", icon: <Globe size={20} /> },
  ];
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {navLinks.map((link) => {
            ...
            return (
                <Link key={link.name} href={`${link.path}?userId=${userId}`} passHref>
                  <motion.div
                    whileHover={{ scale: 1.1, backgroundColor: isActive ? 'rgba(255, 255, 255, 0.25)' : 'rgba(255, 255, 255, 0.15)' }}
                    whileTap={{ scale: 0.95 }}
                    className={cn(
                        "flex items-center gap-2 text-white px-3 py-1.5 rounded-full cursor-pointer transition-colors",
                        isActive ? "bg-white/25 font-semibold" : "bg-transparent" // Conditional class for highlighting
                    )}
                  >
                    {link.icon}
