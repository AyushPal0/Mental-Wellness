...
class APITester:
    ...
    def test_community_endpoints(self):
        ...
        if self.test_user_id:
            ...
            if result and 'post' in result and '_id' in result['post']:
                self.test_post_id = result['post']['_id']

        # Test like post (if we have post and user IDs)
        if self.test_post_id and self.test_user_id:
            like_data = {"user_id": self.test_user_id}
            self.test_endpoint("POST", f"/api/community/posts/{self.test_post_id}/like", like_data, description="Like a post")
        ...
        if self.test_post_id and self.test_user_id:
            ...
            self.test_endpoint("POST", f"/api/community/posts/{self.test_post_id}/comment", comment_data, description="Comment on a post")
    ...
    def generate_report(self):
        ...
        self.log(f"📈 Success Rate: {(self.results['passed_tests']/self.results['total_tests']*100):.1f}%".1f"
        self.log("\n" + "-"*60)
