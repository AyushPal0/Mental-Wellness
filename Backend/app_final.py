...
class MentalWellnessApp:
    ...
    # Removed custom JSON encoder - will handle serialization manually
    ...
    def register_blueprints(self):
        ...
        try:
            ...
            self.app.register_blueprint(game_bp, url_prefix="/api/game")
            self.initialization_status['game_routes'] = True
            self.logger.info("✅ Game routes registered")
        except Exception as e:
            self.logger.error(f"❌ Game routes registration failed: {e}")

        # Register the personality routes
        try:
            from routes.personality_routes_complete import personality_bp
